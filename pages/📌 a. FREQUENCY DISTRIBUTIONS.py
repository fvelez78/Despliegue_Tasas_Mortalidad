...
showData=st.multiselect("### FILTRO",summary_table.columns,default=["Frequency","Percentage Frequency","Cumulative Frequency","Relative Frequency","Cumulative Relative Frequency"]) 
...
fig = px.histogram(df['UnitPrice'], y=df['UnitPrice'], nbins=10, labels={'UnitPrice': 'Precio unitario', 'count': 'Frequencia'}, orientation='h')

# Add a dashed line for mean and customize its appearance
fig.add_hline(y=mean_unitprice, line_dash="dash", line_color="green", annotation_text=f"Precio Unitario Medio: {mean_unitprice:.2f}", annotation_position="bottom right", annotation_font_color="#8B0000")

# Customize marker and line for bars
fig.update_traces(marker=dict(color='#87CEFA', line=dict(color='rgba(33, 150, 243, 1)', width=0.5)), showlegend=True, name='UnitPrice')
