...
def plot_ogives(data, median_val):
    ...
    fig.update_layout(
        title='Ojivas Mayor y Menor, con Medianas',
        xaxis_title='Edad',
        yaxis_title='Frecuencias Acumuladas',
...
def main():
    st.title(" Análisis de Edad: Datos no Agrupados")
    ...
    b1, b2 = st.columns(2)  
    # Display results
    st.subheader("Estadísticas de Datos no Agrupados")
    a1.metric("1st Cuartil (Q1):", f"{Q1:.2f}")
    a2.metric("3rd Cuartil (Q3):", f"{Q3:.2f}")
    a3.metric("Rango Intercuartílico (IQR):", f"{IQR:.2f}")
    b1.metric("Valor Minimo:", f"{min_val:.2f}")
    b2.metric("Valor Maximo:", f"{max_val:.2f}")
    style_metric_cards(border_left_color="#e1ff8b",background_color="#F0FFF0")
