...
try:
 st.header("TENDENCIAS EMPRESARIALES POR GEORREFERENCIACIÓN ")
 items= load_df['Name'].count()
 total_price= float(load_df['TotalPrice'].sum())

 with st.expander("ANALÍTICA"):
  a1,a2=st.columns(2)
  a1.metric(label="Sucursales de la oficina",value=items,help=f""" Precio total: {total_price} """, delta=total_price)
  a2.metric(label="Precio total",value=total_price,help=f""" Precio total: {total_price} """, delta=total_price)
...
#create a map
 ...
 with st.expander(" CARTOGRAFÍA DE OFICINAS & ANÁLISIS"):
  m = add_google_maps(m)
 ...
 with st.expander("SELECCIONAR DATOS"):
  selected_city = st.selectbox("Seleccione una ciudad", df['Name'])
...
#Graphs
 ...
 with col1:
  ...
  fig2 = go.Figure(
    ...
    layout=go.Layout(
        title=go.layout.Title(text="TIPO DE NEGOCIO POR CUARTILES DE INVERSIÓN"),
 ...
 # Create a donut chart
  fig = px.pie(df, values='TotalPrice', names='Name', title='Precio total por Nombre')
  fig.update_traces(hole=0.4)  # Set the size of the hole in the middle for a donut chart
  fig.update_layout(width=800)
  st.plotly_chart(fig, use_container_width=True)
  
except:
  st.error("No se puede mostrar nulo, seleccione al menos una ubicación comercial")
